...
def generate_table(mesh: om.PolyMesh, idx: int, prev_idx: int, depth: int) -> int:
    ...
    for f in mesh.faces():
        ...
        for vert in mesh.fv(f):
            ...
            f_data.append(mesh.vertex_property("prev_idx", vert) + idx)
        # print()
    ...
    if depth == 0:
        fidx = idx + mesh.vertices().__len__()
    else:
        fidx = idx
    ...
    for v in mesh.vertices():
        ...
        for face in mesh.vf(v):
            ...
            v_data.append(mesh.face_property("prev_idx", face) + fidx)
        for edge in mesh.ve(v):
            ...
            if v0 != v:
                ...
                v_data.append(mesh.vertex_property("prev_idx", v0) + idx)
            if v1 != v:
                # v_data.append(v1.idx() + idx)
                v_data.append(mesh.vertex_property("prev_idx", v1) + idx)
            val += 1
        # print()
