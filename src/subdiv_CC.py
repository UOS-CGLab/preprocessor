...
def subdivision3(mesh: om.PolyMesh, prev_idx: int, depth: int, output_dir: str) -> (om.PolyMesh, int):
    ...
    for f in mesh.faces():
        ...
        for h in mesh.fh(f):
            ...
            new_tex_coords = [v1_tex_coord, v2_tex_coord, v3_tex_coord, v0_tex_coord]

            last_face_handle = mesh_next.face_handle(mesh_next.faces().__len__() - 1)

            for i, e in enumerate(mesh_next.fv(last_face_handle)):
                mesh_next.set_texcoord2D(e, new_tex_coords[i])
